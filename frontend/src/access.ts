...
export function navForRole(role?: string): NavItem[] {
  if (role === 'EMPLOYEE') {
    return [
      ...
      { key: 'tickets', href: '/tickets', label: 'My tickets', resource: 'support-tickets', badgeKey: 'tickets' },
  ...
  if (role === 'MANAGER') {
    return [
      ...
      { key: 'requests', href: '/requests', label: 'Requests', resource: 'asset-requests', badgeKey: 'requests' },
      { key: 'tickets', href: '/tickets', label: 'Team tickets', resource: 'support-tickets', badgeKey: 'tickets' },
  ...
  const it: NavItem[] = [
    ...
    { key: 'employees', href: '/employees', label: 'Employees', resource: 'employees', badgeKey: 'employees' },
  ];
  ...
  it.push(
    ...
    { key: 'requests', href: '/requests', label: 'Requests', resource: 'asset-requests', badgeKey: 'requests' },
    { key: 'maintenance', href: '/maintenance', label: 'Maintenance', resource: 'maintenance', badgeKey: 'maintenance' },
    { key: 'tickets', href: '/tickets', label: 'Support Tickets', resource: 'support-tickets', badgeKey: 'tickets' },
    { key: 'reports', href: '/reports', label: 'Reports', resource: 'reports' },
  );
  if (role === 'SUPER_ADMIN' || role === 'IT_ADMIN') {
...
export function canAccessResource(role: string | undefined, resource: string, action: string): boolean {
  const items = navForRole(role);
  if (items.some((i) => i.resource === resource)) {
    if (action === 'create' || action === 'edit' || action === 'delete') {
      if (resource === 'assets') return can(role, action === 'delete' ? 'asset:delete' : 'asset:create');
