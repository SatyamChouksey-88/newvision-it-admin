...
- **axe `aria-hidden-focus`** is on again; Ant Design dropdown/modal portals **and** `.ant-table-measure-row` are excluded because they are upstream focus-trap / measure-row bugs, not our chrome. Measure-row checkboxes are also `display: none` so they leave the a11y tree. Amber status chips use `#92400e` on `#fffbeb`; category “All” pills use `#0958d9` (not `#1677ff`) so 12px white text meets WCAG AA.
